...
def CEX_GEN(json_data):
    ...
    while (True):
        ...
        diag, target_flag = Bounded_Greedy_DFS_Prob(model, min_max_dict, P, diag, target_var, target_index, target_value)
        if target_flag:
            t_flag = True
        #diag = post_process(diag, target_index, target_value)
        if (len(diag.nodes) + len(diag.edges) > 0) and t_flag:
            print('P = ' + str(P))
            print('K = ' + str(K))
...
def Bounded_Greedy_DFS_Prob(model, min_max_dict, P, diag, target_var, target_index, target_value):
    target_flag = False
    ...
    visited = set()
    stack = []
    stack.append((init_node, 0, enabled(model, init_node, 0, P)))
       ...
       # n_prime <-- successor(n, r)
        ...
        if check_state(dst_var_values, min_max_dict):
            ...
            if n_prime.var_values not in visited:
                stack.append((n_prime, p_prime, enabled(model, n_prime, p_prime, P)))
            # stack.append((n_prime, p_prime, enabled(model, n_prime, p_prime, P)))
    
    return (diag, target_flag)
...
def enabled(model, curr_node, path_prob, prob_thresh):
    R = []
    ...
    for i, r in enumerate(model.get_reactions_vector()):
        ...
        if rate > 0:
            p = path_prob + log((rate/total_rate), 10)
            if p > prob_thresh:
                R.append((i, p))
