...
class edge: 
	def __init__(self, n1, n2, reaction=1, prob = 0): 
		self.n1 = n1
		self.n2 = n2
...
class graph: 
	...
	def to_file(self, file_name_prefix, model, model_name): 
		...
		with open(states_file_name, mode='w', encoding='ascii') as f:
			...
			for i, n in enumerate(self.node_list): 
				...
				#set an index for each node for sync between .sta and .tra
				n.set_index(i)
				line = str(i) + ':('
		...
		for i, n in enumerate(self.node_list):
			...
			for e in n.out_edges:
				if (model_name=="circuit0x8E"):
					rate = model.reaction_propensities(var_values)[e.reaction-1]
				
				else:
					comb = 1
					# 2R1 + R2 --K--> R3. the rate would be: #(R1) * #(R1) * #(R2) * K
					for it, r in enumerate(model.reactions_dict()[e.reaction][0]):
						for c in range(r): 
			...
			for j, r in enumerate(model.reactions_dict().values()):
				if (model_name=="circuit0x8E"):
					rate = model.reaction_propensities(var_values)[j]
	...
	def model_check(self, model, model_name, prism_bin, csl_prop):
		self.to_file('./results/' + model_name + '/' + model_name, model, model_name)
